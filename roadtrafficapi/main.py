...
@use_kwargs(
    {"ward_id": fields.Int(location="query", required=False, missing=None)}
)
def aadf_by_direction_list(
    page, longitude, latitude, distance, ward_id, **kwargs
):
    """
    ...
    if ward_id:
        # Opted to use ON clause instead of defining relationship in model as
        # hesitant to modify table structure generated by `shp2pgsql`.
        q = q.join(Ward, Ward.geom.ST_Contains(AADFByDirection.point))
        q = q.filter(Ward.gid == ward_id)
